...
- **Every State File write goes through `editStateFile`** (`app/src/edit.ts`); no other production code calls `workspace.saveStateFile`, and the only other callers are the two Workspace adapters' own tests, which are testing the port itself. An editing use case is a pure `state -> state` function in `core` plus a label it hands that wrapper — recording a Pick, creating a Variant, adding an Attempt, setting a Blocked Time. The wrapper is the only place that carries the revision a save was based on, so a second write path is a hole in the external-edit guard, and the only place that puts the previous value on the undo stacks `server/src/history.ts` holds, so a second one is an edit that cannot be undone. Undo is that same wrapper putting an earlier snapshot back, never an inverse (ADR-0013).
