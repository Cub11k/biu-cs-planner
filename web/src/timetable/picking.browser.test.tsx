...
/** The tile for one Group, found by the detail line `tileText` writes on it. */
function tileFor(mounted: HTMLElement, groupNumber: string): HTMLElement {
  const found = [...mounted.querySelectorAll<HTMLElement>(".day-column .tile")].find((tile) =>
    tile.textContent?.includes(`89-110 · Lecture · ${groupNumber}`),
  );
  if (found === undefined) throw new Error(`no tile for Group ${groupNumber}`);
  return found;
}
