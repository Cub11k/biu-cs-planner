...
/**
 * One ask. Reports a change only when the counter differs from a counter it has already
 * seen: the first answer is the baseline, so a page that loads against a Workspace which
 * has changed nine times today does not reload itself the moment it opens.
 *
 * A number, never a value with meaning: the server restarts it at 0, so only the movement
...
export function workspaceChangePoll(
  ...
  return async (): Promise<void> => {
    ...
    try {
      ...
      const moved = seen !== undefined && revision !== seen;
      seen = revision;
      if (moved) onChanged();
    } catch {
...
export function watchWorkspaceChanges(
  ...
  const poll = workspaceChangePoll(client, onChanged);
  void poll();
  return (options.repeat ?? realRepeat)(() => void poll(), options.everyMs ?? DEFAULT_EVERY_MS);
}

/**
 * How many times the Workspace has changed since this page loaded. Not the server's
 * counter: a screen only needs to know that the number moved, and one that starts at 0 on
 * every page load cannot be mistaken for a version of anything.
 *
...
export function useWorkspaceChanges(
  options: WorkspaceChangesOptions & { client?: ApiClient } = {},
): number {
  ...
  useEffect(
    () =>
      watchWorkspaceChanges(client, () => setChanges((count) => count + 1), {
