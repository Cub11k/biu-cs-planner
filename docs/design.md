...
- **Re-import:** before confirming, the importer shows what changed (Groups added, removed, moved). A part speaks for the Offerings it carries rows for, and for the same Course's Offerings whose Semesters its rows overlap; Groups it does not name there are superseded, with a Warning. See [ADR-0011](adr/0011-a-part-speaks-for-the-offerings-it-carries-rows-for.md). Afterwards, every Variant is compared against the new Catalog using its Pick snapshots:
