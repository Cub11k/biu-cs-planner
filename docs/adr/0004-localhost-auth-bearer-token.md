...
- **Storage:** the token lives in the user config directory, never the Workspace, and stays stable across restarts. One token per user account on the machine, with no expiry and nothing that retires it on its own, so the URL the launcher prints stays a working bookmark.
